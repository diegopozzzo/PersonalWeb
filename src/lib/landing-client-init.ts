 ...
 * Regenerate with: `node scripts/extract-landing-js.mjs && node scripts/compose-landing-init.mjs`
 */
...
function initDbpaMobileNav() {
  ...
  function currentLangNav() {
    return window.__dbpaLang === "es" ? "es" : "en";
  }

  function ariaOpen() {
    return currentLangNav() === "es" ? "Abrir menú de navegación" : "Open navigation menu";
  }

  function ariaClose() {
    return currentLangNav() === "es" ? "Cerrar menú de navegación" : "Close navigation menu";
  }

  function ariaPanel() {
    return currentLangNav() === "es" ? "Navegación del sitio" : "Site navigation";
  }

  function labelMenuSr() {
    return currentLangNav() === "es" ? "Menú" : "Menu";
  }

  function applyNavMobileAria() {
    var openNow = toggle.getAttribute("aria-expanded") === "true";
    toggle.setAttribute("aria-label", openNow ? ariaClose() : ariaOpen());
    panel.setAttribute("aria-label", ariaPanel());
    var sr = toggle.querySelector(".sr-only");
    if (sr) sr.textContent = labelMenuSr();
  }

  function setOpen(open) {
    toggle.setAttribute("aria-expanded", open ? "true" : "false");
    toggle.setAttribute("aria-label", open ? ariaClose() : ariaOpen());
  ...
  window.addEventListener(
    "dbpa:langchange",
    /** @param {CustomEvent<{ lang?: string }>} ev */
    function () {
      applyNavMobileAria();
    }
  );
